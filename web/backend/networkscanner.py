...
options = None
class Host:
    def __init__(self, ip, mac=None, os=None):
        self.ip = ip
        self.mac = mac
        self.os = os
...
class NmapScanner():
    ...
    def arp_pingsweep(self, ip):
        ...
        for host, status in hosts_list:
            ...
            try:
                new_hosts[host].mac = scanner[host]['addresses']['mac'].lower()
            except KeyError:
...
class RemoteLogin():
    ...
    def ssh(self):
        ...
        ssh_stdin, ssh_stdout, ssh_stderr = ssh_session.exec_command(
            "chmod +x " + remote_file_location)

        ssh_stdin, ssh_stdout, ssh_stderr = ssh_session.exec_command(
            scanner_exec_command)
        # ssh_stdin.write(cred['pass']+'\n')
        # ssh_stdin.flush()
